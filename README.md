...
[![Project-Maven-1.png](https://i.postimg.cc/fR0ZtG4T/Project-Maven-1.png)](https://postimg.cc/v4MCCSHp)
...
- Выбираем **Catalog**. Предлагаю остаивить по умолчанию - internal
...
Скорее всего после сообщения об успешной сборке проекта мы также увиидим и предупреждения (WARNINGS) о том, что используется старый плагин Maven.
...
Поэтому сейчас нажимаем на три точки справа от пути, находим куда мы устанавливали Maven, выбираем файл, т.о. прописываем новый путь до Maven и нажимаем **ОК**. Убеждаемся, что версия под строкой пути
поменялась на новую, нажимаем **Apply** > **OK** и смело закрываем окно. Теперь можем дальше работать с проектом.
