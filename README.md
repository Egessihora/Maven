...
Эта задача теперь также выполняется автоматически. Нам вообще не нужно писать какие-то команды в терминале, что-то указывать (какие директории нужно скомпилировать, куда положить скомпилированные ресурсы). Всё делается автоматически с использованием **принципа соглашения над конфигурацией**. Весь процесс деплоя упрощается в разы. Это всё также относится и к **выполнению тестов**.
...
*Принцип "Соглашение важнее конфигурации" (convention over configuration)
